...
class MarketTrackerController(QObject):
    ...
    def poll(self) -> None:
        ...
        self._feed = str(self.settings.value("alpaca/feed", DEFAULT_FEED) or DEFAULT_FEED).strip().lower()
...
class MarketTrackingPage(QWidget):
    ...
    def __init__(self, database_path) -> None:
        ...
        subtitle = QLabel(
            "HIGH ATTENTION tickers are automatically observed for up to 4 hours. TrendVision discovers them; Alpaca measures what happens afterward."
        )
        ...
        self.table.setHorizontalHeaderLabels(
            [
                "Started", "Ticker", "Trigger", "Reference", "Last", "Return",
                "MFE", "MAE", "Bid", "Ask", "Spread", "Samples", "Feed / Status",
            ]
        )
        ...
        note = QLabel(
            "Reference price is the first successful Alpaca snapshot after HIGH ATTENTION is detected, not necessarily the exact millisecond of the TrendVision alert. MFE/MAE are descriptive measurements, not profit guarantees."
        )
    ...
    def refresh(self) -> None:
        ...
        mfe_values = [float(row["mfe_pct"]) for row in metrics if row.get("mfe_pct") is not None]
        ...
        for row_index, row in enumerate(metrics):
            ...
            feed_status = f"{str(row.get('feed') or '-').upper()} / {row.get('status') or '-'}"
...
class MarketSystemPage(cal.ai.base.SystemPage):
    def __init__(self, database_path) -> None:
        ...
        self.feed_combo.addItem("SIP — consolidated US exchanges (subscription required)", "sip")
        self.feed_combo.addItem("Delayed SIP — consolidated, 15 min delayed", "delayed_sip")
        current_feed = str(self.market_settings.value("alpaca/feed", DEFAULT_FEED) or DEFAULT_FEED)
    ...
    def _refresh_alpaca_status(self) -> None:
        feed = str(self.market_settings.value("alpaca/feed", DEFAULT_FEED) or DEFAULT_FEED).upper()
        if get_alpaca_credentials():
            self.alpaca_status.setText(f"Alpaca credentials configured. Selected feed: {feed}.")
        else:
    ...
    def _save_alpaca(self) -> None:
        try:
            ...
            self.alpaca_status.setText(f"Could not save Alpaca credentials: {type(exc).__name__}: {exc}")
            return
    ...
    def _save_feed(self) -> None:
        self.market_settings.setValue("alpaca/feed", self.feed_combo.currentData() or DEFAULT_FEED)
        self._refresh_alpaca_status()
...
class MarketMainWindow(cal.CalibrationMainWindow):
    def __init__(self) -> None:
        ...
        self.market_controller = MarketTrackerController(self.repo, self.config.database_path)
